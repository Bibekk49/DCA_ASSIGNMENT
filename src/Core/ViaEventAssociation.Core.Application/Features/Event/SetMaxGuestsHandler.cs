...
public class SetMaxGuestsHandler : ICommandHandler<SetMaxGuestsCommand>
{
    private readonly IEventRepository _repo;
    private readonly IUnitOfWork _uow;

    public SetMaxGuestsHandler(IEventRepository repo, IUnitOfWork uow)
    {
        _repo = repo;
        _uow = uow;
    }
    ...
    public async Task<Result> HandleAsync(SetMaxGuestsCommand command)
    {
        ...
        var result = evt.SetMaxGuests(command.MaxGuests);
